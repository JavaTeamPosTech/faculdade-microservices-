...
public class AuthClientService {
    ...
    /**
     * Busca os detalhes completos do Usuario (nome, email, telefone) no ms-autenticacao.
     * @param userId O ID do Usuario.
     * @return UsuarioDetails.
     * @throws RecursoNaoEncontradoException se o usuário não for encontrado (404).
     */
    public UsuarioDetails buscarUsuarioPorId(UUID userId) {
        String url = authServiceBaseUrl + "/" + userId.toString();
        log.info("RPC INICIADO: Buscando Usuario ID {} via GET {}", userId, url);

        try {
            UsuarioDetails response = webClient.get()
                    .uri(url)
                    ...
                    .bodyToMono(UsuarioDetails.class)
                    .block(); // Chamada síncrona

            log.info("RPC SUCESSO: Detalhes do Usuario ID {} recebidos.", userId);
            return response;
