...
public class CustomAuthenticationEntryPoint implements AuthenticationEntryPoint {
    ...
    public void commence(HttpServletRequest request,
        ...
        response.setHeader(HttpHeaders.WWW_AUTHENTICATE, "Bearer realm=\"meuhospital-agendamento\"");

        var errorResponse = Map.of(
                "timestamp", LocalDateTime.now().toString(),
                "status", String.valueOf(HttpServletResponse.SC_UNAUTHORIZED),
                "error", "Unauthorized",
                "message", "Token ausente ou inválido no agendamento: " + authException.getMessage(),
